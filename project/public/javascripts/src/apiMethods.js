...
async function getUserScheduledById(scheduleId) {
  try {
    const { usr_id } = getLocalStorageUser();

    const result = await $.ajax({
      url: `api/users/${usr_id}/schedule/${scheduleId}`,
      method: "get",
      dataType: "json",
    });
...
async function completeUserCircuit(scheduleId) {
  try {
    const { usr_id } = getLocalStorageUser();
    const result = await $.ajax({
      url: `/api/users/${usr_id}/schedule/${scheduleId}/complete`,
      method: "put",
      dataType: "json",
    });
    return result;
